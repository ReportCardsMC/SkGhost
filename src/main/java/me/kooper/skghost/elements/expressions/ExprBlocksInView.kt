...
class ExprBlocksInView : SimpleExpression<Location>() {
    ...
    companion object {
        init {
            Skript.registerExpression(
                ExprBlocksInView::class.java,
                Location::class.java, ExpressionType.COMBINED, "[the] blocks in %view% (of|in) %stage%", "all blocks in %view% (of|in) %stage%"
            )
        }
    }

    override fun toString(event: Event?, debug: Boolean): String {
        return "Blocks in view expression with expression view: ${view.toString(event, debug)}, expression stage: ${stage.toString(event, debug)}"
    }
